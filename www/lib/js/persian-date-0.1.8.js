...
function persianArrayToGregorianDate(parray) {
  ...
  var pd = calcPersian(parray[0] ? parray[0]*1 : 0, parray[1] ? parray[1]*1 : 1, parray[2] ? parray[2]*1 : 1);
  var gDate = new Date(pd[0], pd[1], pd[2]);
  gDate.setYear(pd[0]);
  gDate.setMonth(pd[1]);
  gDate.setDate(pd[2]);
  // TODO:
  gDate.setHours(parray[3] ? parray[3]*1 : 0);
  gDate.setMinutes(parray[4] ? parray[4]*1 : 0);
  gDate.setSeconds(parray[5] ? parray[5]*1 : 0);
  return gDate;
}
...
PersianDate.prototype = {
  ...
  unix: function (timestamp) {
	...
	fromLong: function (datetimefa) {
	if (datetimefa) {	
		sec=datetimefa%100;datetimefa=Math.floor(datetimefa/100);
		mino=datetimefa%100;datetimefa=Math.floor(datetimefa/100);
		hour=datetimefa%100;datetimefa=Math.floor(datetimefa/100);
		day=datetimefa%100;datetimefa=Math.floor(datetimefa/100);
		mon=datetimefa%100;datetimefa=Math.floor(datetimefa/100);
		year=datetimefa;
	
    
      return new persianDate([year,mon,day,hour,mino,sec]);
    } else {
...
persianDate.fromLong = persianDate.prototype.fromLong;
persianDate.monthRange=function(){return monthRange;}
